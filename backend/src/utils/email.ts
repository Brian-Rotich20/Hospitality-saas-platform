// src/utils/email.ts
// Resend email service — free tier 3,000/month
// Uses testing email during development

import { Resend } from 'resend';

const resend = new Resend(process.env.RESEND_API_KEY);

// Use your verified sender — on free tier use onboarding@resend.dev
// Once you add a domain, change to noreply@linkmart.co.ke
const FROM = process.env.EMAIL_FROM ?? 'LinkMart <onboarding@resend.dev>';
const APP_URL = process.env.NEXT_PUBLIC_APP_URL ?? 'https://linkmart-olive.vercel.app';
...
export async function sendVendorVerificationEmail(opts: {
  ...
  return resend.emails.send({
    from:    FROM,
    to:      opts.to,
    subject: `${opts.otp} — Verify your LinkMart vendor account`,
    html: `
      <!DOCTYPE html>
      <html>
            ...
            <table width="480" cellpadding="0" cellspacing="0"
              ...
              <tr>
                <td style="background:#2D3B45;padding:28px 32px;">
                  <p style="margin:0;font-size:20px;font-weight:900;color:#F5C842;
                    letter-spacing:-0.03em;">LinkMart</p>
                  <p style="margin:4px 0 0;font-size:11px;color:#ffffff80;
                    text-transform:uppercase;letter-spacing:0.1em;">Vendor Portal</p>
                </td>
              </tr>
              ...
              <tr>
                <td style="padding:32px;">
                  <p style="margin:0 0 8px;font-size:22px;font-weight:900;color:#111827;">
                    Verify your email
                  </p>
                  <p style="margin:0 0 24px;font-size:14px;color:#6B7280;line-height:1.6;">
                    Hi <strong style="color:#111827;">${opts.businessName}</strong>, thanks for applying
                  ...
                  <p style="margin:0 0 24px;font-size:13px;color:#6B7280;line-height:1.6;">
                    Once verified, your vendor account will be <strong style="color:#059669;">
                    instantly activated</strong> and you can start creating listings.
                  </p>
...
export async function sendVendorApprovedEmail(opts: {
  ...
  return resend.emails.send({
    from:    FROM,
    to:      opts.to,
    subject: `🎉 Your LinkMart vendor account is approved!`,
    html: `
      <!DOCTYPE html>
      <html>
      <body style="margin:0;padding:0;background:#F8FAFC;font-family:'DM Sans',system-ui,sans-serif;">
        <table width="100%" cellpadding="0" cellspacing="0" style="padding:40px 16px;">
          <tr><td align="center">
            <table width="480" cellpadding="0" cellspacing="0"
              ...
              <tr>
                <td style="background:#2D3B45;padding:28px 32px;">
                  <p style="margin:0;font-size:20px;font-weight:900;color:#F5C842;">LinkMart</p>
                </td>
              </tr>
              <tr>
                <td style="padding:32px;">
                  ...
                  <p style="margin:0 0 24px;font-size:14px;color:#6B7280;line-height:1.6;">
                    Congratulations <strong style="color:#111827;">${opts.businessName}</strong>!
...
export async function sendVendorRejectedEmail(opts: {
  ...
  return resend.emails.send({
    from:    FROM,
    to:      opts.to,
    subject: `Update on your LinkMart vendor application`,
    html: `
      <!DOCTYPE html>
      <html>
      <body style="margin:0;padding:0;background:#F8FAFC;font-family:'DM Sans',system-ui,sans-serif;">
              ...
              <tr>
                <td style="padding:32px;">
                  ...
                  <p style="margin:0 0 16px;font-size:14px;color:#6B7280;line-height:1.6;">
                    Hi <strong style="color:#111827;">${opts.businessName}</strong>, unfortunately
                  ...
                  <p style="margin:0;font-size:13px;color:#6B7280;line-height:1.6;">
                    You're welcome to re-apply after addressing the above. If you have questions,
                    reply to this email.
                  </p>
                </td>
              </tr>
                ...
                <td style="padding:32px;">
                  ...
                  <table width="100%" style="background:#F8FAFC;border-radius:10px;
                    ...
                    <tr>
                      <td style="padding:6px 0;">
                        <p style="margin:0;font-size:11px;font-weight:700;color:#9CA3AF;
                          text-transform:uppercase;">Service</p>
                        <p style="margin:2px 0 0;font-size:14px;font-weight:700;color:#111827;">
                          ${opts.listingTitle}</p>
                      </td>
                    </tr>
                    ...
                    <tr>
                      <td style="padding:6px 0;">
                        <p style="margin:0;font-size:11px;font-weight:700;color:#9CA3AF;
                          text-transform:uppercase;">Dates</p>
                        <p style="margin:2px 0 0;font-size:13px;color:#374151;">
                          ${opts.startDate} → ${opts.endDate}</p>
                      </td>
                    </tr>
                    <tr><td style="height:1px;background:#E5E7EB;"></td></tr>
                    <tr>
...
export async function sendNewBookingNotificationEmail(opts: {
  ...
  return resend.emails.send({
    from:    FROM,
    to:      opts.to,
    subject: `New booking request — ${opts.listingTitle}`,
    html: `
      <!DOCTYPE html>
      <html>
      <body style="margin:0;padding:0;background:#F8FAFC;font-family:'DM Sans',system-ui,sans-serif;">
